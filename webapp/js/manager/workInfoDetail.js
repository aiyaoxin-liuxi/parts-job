...
//		params.releaseType = $("input[name='releaseType']:checked").val(); // 发布方式
		...
		$.ajax({
			type: "post",
			url: _basePath + "/sys/updateJobInfo",
			data: params,
			dataType:"json",
			async: true, //同步方式
			success: function(data) {
				data = $.parseJSON(data);
				if(data.success){
					window.location.href = "workInfoList";
				} else {
					alert(data.errmsg);
...
// 返回列表
function reList(state, pageNo, pageSize){
	window.location.href= _basePath + '/sys/workInfoList?state='+ state + "&pageNo=" + pageNo + "&pageSize=" + pageSize;
